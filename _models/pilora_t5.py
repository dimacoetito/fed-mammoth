...
class PiLora(BaseModel):
    def __init__(
        ...
        self.cur_A = {}
        self.cur_B = {}
        #self.network.model.encoder.block[0].layer[0].SelfAttention.q
        #self.network.model.encoder.block[0].layer[0].SelfAttention.k
        #self.network.model.encoder.block[0].layer[0].SelfAttention.v
        #self.network.state_dict()['model.encoder.block.0.layer.0.SelfAttention.q.weight']
        #self.network.state_dict()['model.encoder.block.0.layer.0.SelfAttention.k.weight']
        #self.network.state_dict()['model.encoder.block.0.layer.0.SelfAttention.v.weight']
    ...
    def forward(self, x, fabric=True):
        ...
        prelogits = functional_call(self.network, opt_dict, x, kwargs={'prelogits' : True})['last_hidden_state'][:, 0]
    ...
    def set_requires_grad(self):
        ...
        for task in range(self.cur_task):
            for key in self.lora_keys:
                self.old_A[task][key] = self.old_A[task][key].detach()
                self.old_B[task][key] = self.old_B[task][key].detach()
            for param in self.class_protos[task]:
                param.requires_grad = False
        for param in self.class_protos[self.cur_task]:
            param.requires_grad = True
        ...
        with self.fabric.autocast():
            ...
            if self.cur_task > 0:
                for i in range(self.cur_task):
                    ...
                    loss_ort += torch.abs(torch.mm(self.cur_A['model.encoder.block.0.layer.0.SelfAttention.q.weight'], self.old_A[i]['model.encoder.block.0.layer.0.SelfAttention.q.weight'].T)).sum() + torch.abs(torch.mm(self.cur_A['model.encoder.block.0.layer.0.SelfAttention.v.weight'], self.old_A[i]['model.encoder.block.0.layer.0.SelfAttention.v.weight'].T)).sum()
            loss += 0.5 * loss_ort
    ...
    def begin_task(self, n_classes_per_task: int):
        ...
        if self.cur_task == 0:
            for t in range(self.num_tasks):
                self.class_protos[t] = nn.ParameterList([nn.Parameter(0.1*torch.randn(1, 512), requires_grad=False).to(self.device) for i in range(self.cpt)])
        #blk : _LoRA_qkv_timm = self.network.model.blocks[0].attn.qkv
        #blk.change_lora(self.cur_task)
        if self.cur_task > 0:
            self.old_A[self.cur_task - 1] = deepcopy(self.cur_A)
        ...
        self.cur_round = 0
        self.average_features = torch.zeros(self.cpt, 512).to(self.device)
    ...
    def begin_round_client(self, dataloader: DataLoader, server_info: dict):
        ...
        pars = [self.cur_A[key] for key in self.lora_keys] + [self.cur_B[key] for key in self.lora_keys]
    ...
    def end_round_client(self, dataloader: DataLoader):
        ...
        self.optimizer = None
        classes = []
        average_features = torch.zeros(self.cpt, 512).to(self.device)
    ...
    def get_client_info(self, dataloader: DataLoader):
        client_info = {}
        client_info["cur_A"] = deepcopy(self.cur_A)
        client_info["cur_B"] = deepcopy(self.cur_B)
    ...
    def end_round_server(self, client_info: List[dict]):
        with torch.no_grad():
            ...
            for key in self.lora_keys:
                self.cur_A[key] = nn.Parameter(torch.tensor(torch.stack([client["cur_A"][key] * norm_weight for client, norm_weight in zip(client_info, norm_weights)]).sum(0), device=self.device, requires_grad=False))
                self.cur_B[key] = nn.Parameter(torch.tensor(torch.stack([client["cur_B"][key] * norm_weight for client, norm_weight in zip(client_info, norm_weights)]).sum(0), device=self.device, requires_grad=False))
            eps = 1e-10
            for c in total_classes:
                ...
                for client in client_info:
                    if c in client["classes"]:
                        ...
                        feats = torch.cat((feats, client["avg_feat"][idx].unsqueeze(0).to(self.device))) #shape (num_clients_w_feature, 512)
                protos = torch.stack([client["proto"][num_c].to(self.device) for client in client_info]).squeeze(1) #shape (num_clients, 512)
